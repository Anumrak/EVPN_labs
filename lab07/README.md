...
5) Настроить вторичный IP адрес на интерфейсе Loopback 0 (Anycast vPC IP), одинаковый для Leaf_1 и Leaf_2.
...
Без вторичного адреса на интерфейсе NVE (Loopback), vPC проверка конфигурации не соберется, ругнувшись на его отсутствие.
```
...
![Снимок3](https://github.com/Anumrak/EVPN_labs/assets/133969023/6033e1db-d993-4d1a-b6e0-e83b97af6c32)
...
Чтобы изменить это поведение, можно использовать функцию маршрутизации от SVI интерфейса используемого vlan, внутри которого ходит трафик между peer-link'ами. Включаем эту функцию командой peer-gateway внутри vPC домена. Но и это еще не все. Так как трафик внутри peer-link теперь маршрутизируется, значит кроме подмены мак адресов источника он теперь еще и TTL уменьшает.
